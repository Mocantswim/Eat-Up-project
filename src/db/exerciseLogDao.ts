...
export async function addExercise(params: ExerciseInput): Promise<number> {
  ...
  const result = await db.runAsync(
    `INSERT INTO exercise_log (date, sport_type, kind, duration_min, rep_count, load_weight, met_value, weight_used, per_unit_kcal, calories, created_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    params.date,
    params.sportType,
    params.kind,
...
export async function updateExercise(id: number, params: ExerciseInput): Promise<void> {
  ...
  const db = getDb();
  await db.runAsync(
    `UPDATE exercise_log SET sport_type=?, kind=?, duration_min=?, rep_count=?, load_weight=?, met_value=?, weight_used=?, per_unit_kcal=?, calories=? WHERE id=?`,
    params.sportType,
    params.kind,
    durationMin,
    params.reps ?? null,
